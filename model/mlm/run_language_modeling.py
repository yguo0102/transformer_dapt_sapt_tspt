...
class CustomIterableDataset(IterableDataset):
  def __init__(self, tokenizer, file_path, block_size, batch_size, size, n_gpu):
    self.file_path = file_path
    self.tokenizer = tokenizer
    self.block_size = block_size
    self.len = int(size/(batch_size*n_gpu))
        ...
        for text in file_obj:
            ...
            if len(tokenized_text) <= block_size:
              ...
              for i in range(0, len(tokenized_text) - block_size + 1, block_size):  # Truncate in block of block_size
                ...
                yield torch.tensor(example, dtype=torch.long)
                break # only use the first block
...
def get_dataset(args: DataTrainingArguments, training_args: TrainingArguments, tokenizer: PreTrainedTokenizer, evaluate=False):
    ...
    if args.line_by_line:
        ...
        #    batch_size=training_args.per_device_train_batch_size, size=893347756, n_gpu=training_args.n_gpu
        #)
        # all:856,903,967
        # 1,401,819,651
        # 893347756 RC_2019_merge.shuf.txt
        #return LineByLineTextDataset(tokenizer=tokenizer, file_path=file_path, block_size=args.block_size)
        return TextDataset(
...
def main():
    ...
    ## Load dict
    #mlm_custom_dict_path = 'mlm_custom_dict.txt'
    #mlm_custom_dict = set([])
    #with open(mlm_custom_dict_path) as fr:
    #    for line in fr:
    #        word, _ = line.strip().split('\t')
    #        tokens = tokenizer.encode(word + ' ' + word) # e.g. 'from from'
    #        if len(tokens) == 4:
    #            # tokens is [cls, code1, code2, sep]
    #            mlm_custom_dict.add(tokens[1])
    #            mlm_custom_dict.add(tokens[2])
